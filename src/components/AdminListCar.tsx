...
const AdminListCar: React.FC = () => {
    ...
    const fetchCars = async () => {
        try {
            const response = await fetch("https://synrgy7-bcr-backend.fly.dev/api/v1/cars");
            if (!response.ok) {
    ...
    const handleDelete = async (id: string) => {
        ...
        try {
            const response = await fetch(`https://synrgy7-bcr-backend.fly.dev/api/v1/cars/delete/${id}`, {
                method: "DELETE",
                headers: {
